...
- `getWritable(idx)` to get a writable pointer and user should assgin the object referred to by the pointer. If the object at the index is not ready for writing(when the queue is `full()`) it'll return nullptr, and user should retry.
